  ...
  const onAddToCart = async (obj) => {
    try {
      ...
      if (findItem) {
        ...
        await axios.delete(
          `https://63d9a09f2af48a60a7bd2407.mockapi.io/cart/${findItem.id}`
        );
      } else {
        setCartItems((prev) => [...prev, obj]);
        const { data } = await axios.post(
          "https://63d9a09f2af48a60a7bd2407.mockapi.io/cart",
          obj
        );
        setCartItems((prev) =>
          prev.map((item) => {
  ...
  const onAddToFavorite = async (obj) => {
    try {
      if (favorites.find((favObj) => Number(favObj.id) === Number(obj.id))) {
        axios.delete(
          `https://63d9a09f2af48a60a7bd2407.mockapi.io/favorites/${obj.id}`
        );
        setFavorites((prev) =>
        ...
        const { data } = await axios.post(
          "https://63d9a09f2af48a60a7bd2407.mockapi.io/favorites",
          obj
        );
